...
export class EditTaskEpic {
	...
	static uploadFile = (action$) =>
		action$.ofType(FILE_UPLOAD)
			.switchMap(({payload}) => {
				return Observable.create(oberver => {
					const { completionDate, details, taskTitle, assignTo, workerId, file, adminId, status, timestamp, key } = payload;
					...
					fileStore.on('state_changed',() => {}, (err) => {
						...
						const obj = { fileURL, completionDate, details, taskTitle, assignTo, workerId, timestamp, adminId, status, key };
	...
	static editTask = (action$) => 
		action$.ofType(EDIT_TASK)
			.switchMap(({payload}) => {
				const { key } = payload;
				delete payload.key;
				return Observable.concat(
					getFirebase().update(`all-tasks/${key}`, payload)
				).switchMap((res) => {
