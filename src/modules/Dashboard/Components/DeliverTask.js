...
class DeliverTaskComponent extends Component {
	...
	render() {
		...
		return (
	      ...
	      title={
	      	<div>
	      		<span className='deliver-task-modal-title'>{task !== null ? task.taskTitle : ''}</span>
	      		<span className='deliver-task-modal-time'>
	      			Date Pending: {task !== null ? moment(task.completionDate).format("Do MMM,YYYY") : ''}
	      		</span>
	      	</div>
	      	}
	      modal={false}
...
export let DeliverTask = connect(
	mapStateToProps, 
	{deliverTask}
