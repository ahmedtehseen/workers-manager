                ...
                tasks !== undefined ? (
                  Object.keys(tasks).map((key) => (
                    <ListItem
                      ...
                      leftAvatar={
                        <Avatar src={process.env.PUBLIC_URL + "/task.png"} />
                      }
                    />
                  ))
                ) : (
                  <ListItem
                    primaryText={"No Task Assigned yet"}
                    leftAvatar={
                      <Avatar src={process.env.PUBLIC_URL + "/task.png"} />
                    }
                  />
          ...
          <Paper className="report-tab" zDepth={1}>
            <List>
              ...
              {tasks !== null ? (
                tasks !== undefined ? (
                  Object.keys(tasks)
                    .filter((key) => tasks[key].status === "pending")
                    .map((key) => (
                      <ListItem
                        key={key}
                        primaryText={tasks[key].taskTitle}
          ...
          <Paper className="report-tab" zDepth={1}>
            <List>
              ...
              {tasks !== null ? (
                tasks !== undefined ? (
                  Object.keys(tasks)
                    .filter((key) => tasks[key].status === "completed")
                    .map((key) => (
                      <ListItem
                        key={key}
                        primaryText={tasks[key].taskTitle}
...
const wrappedReport = firestoreConnect()(ReportComponent);
...
export let Report = connect(mapStateToProps, {}, null, { forwardRef: true })(
  wrappedReport
);
